...
const char* enum_as_string[]={"",".jpeg",".png",".zip",".gzip",".dir"};

typedef struct index_data                   //index_data- main structrue for indexing containg all file informations
{
    char file_name[MAX_NAME];               //file name
    char path[MAX_PATH];                    //path to the file
    off_t size;                             //size of the file
    uid_t owner_id;                         //owner id number
    f_type type;                            //type of the file(.jpeg,.png,etc..)
...
typedef struct thread_struct                //thread_struct- structare containg all thread elements used in thread work
{
    pthread_t pth_id;                       //thread id number
    const char* directory;                  //stores the directory on which we want to perform indexing
    const char* filepath;                   //full path to the file in which we want to save informations provided by indexing
    bool is_open;                           //I used it to check wether things are being written to the file (useful in exit functions)
    bool is_indexing;                       //I used it to mark wether the thread is performing indexing or not (for exit! function)
    int reidx_time;                         //that saves info if the auto re-indexing should be performed (if reidx_time!=0) and the clock should be set
    time_t clock;                           //save the time after last re-indexing
...
int indexing(const char* name,const struct stat *s,int type,struct FTW *f);     //function used in nftw which will perform indexing 
...
void finding_info(char* command,command_t type,thread_struct* struct_idx);
void write_to_file(const char* path,index_structure* struct_idx);                   //helper function for writing data to the file with path "path"
void read_a_file(const char* path,index_structure* struct_idx);                     //helper function for reading data from the file with path "path"
...
// ^ helper function for printing in largerthan namepart owner commands
...
{
    int n=0;                                                //variable storing -t argument value
    char* path_d=NULL;                                      //variable storing -d argument value
    char* path_f=NULL;                                      //variable storing -f argument value
    ...
    if(fptr==-1){   
        ...
        read_a_file(path_f,&pth->main);                               //reading file content     
        if(n!=0){
...
{
    struct stat _time;
    if((stat(path,&(_time)))!=0) ERR("fstat()");                //taking file stat info + error handlig
    pthread_mutex_lock(pth->mutex_clock);
    pth->clock=_time.st_mtim.tv_sec;                              //saving information about last file modification in seconds
    pthread_mutex_unlock(pth->mutex_clock);
}

void command_read(thread_struct* pth,char* command){
    while(1) {
        if((fgets(command,MAX_COMMAND,stdin))==NULL) ERR("fgets()");                    //getting a line of user input + error handling
        command[strlen(command)-1]='\0';                                                //fgets end with '\n' so cutting that
        if(strcmp(command,"exit")==0){
            if(pth->is_open==true) { if((pthread_join(pth->pth_id,NULL))!=0) ERR("pthread_join()"); }
            break;                                                                     //getting out of the loop to start terminating program
        } 
        else if(strcmp(command,"exit!")==0){
            pthread_mutex_lock(pth->mutex_indexing);
            if(pth->is_indexing==true){
                if((pthread_cancel(pth->pth_id))!=0) ERR("pthread_cancel()");          //calling clean-up handlers and cancelling the thread
                pthread_mutex_unlock(pth->mutex_indexing);
            }
            else if(pth->is_open==true){
                pthread_mutex_unlock(pth->mutex_indexing);
                if((pthread_join(pth->pth_id,NULL))!=0) ERR("pthread_join()");         //waiting for thread to end (= end of writing to file, as writing to file is last action that thread performs)
            }
...
int indexing(const char* name,const struct stat *s,int type,struct FTW *f)
{
    ...
    { 
        ...
        tempIndex.indexStruct[tempIndex.indexStruct_size-1].owner_id=s->st_uid;                                  //taking information about owner id and saving it to _index
        tempIndex.indexStruct[tempIndex.indexStruct_size-1].size=s->st_size;                                     //taking information about file size and saving it to index_data    
        if(type==FTW_DP)
...
{ 
    ...
    pthread_cleanup_push(cancelling,temp);                                                           //pushing cancelling routine (pointed by cancelling) on the top of the stack   
    ...
    if(temp->reidx_time!=0)                                                                          //in case of re-indexing "switched on"
    {
        pthread_mutex_lock(temp->mutex_clock);
        if((time(&(temp->clock)))==-1) ERR("time()");                                                //saving time from last indexing/re-indexing    
    ...
    pthread_cleanup_pop(0);                                                                          //poping the clean-up handler (indicating where the section of code in which cancelling will be executed ends)
    free(tempIndex.indexStruct);
    if((TEMP_FAILURE_RETRY(fprintf(stdout,"Indexing has finished\n")))<0) ERR("fprintf()");                              //printing info about the indexing ended
    if(pthread_mutex_lock(temp->mutex_open)!=0) ERR("pthread_mutex_unlock()");
    temp->is_open=true;                                                                             //indicating starting saving to the file
    write_to_file(temp->filepath,&(temp->main));                                                                  //writing _index data to file
    temp->is_open=false;                                                                            //indicating the end of saving to the file
...
void* reindexing(void* arguments)
{
    ...
    {
        pthread_mutex_lock(temp->mutex_clock);
        if((time(NULL)-temp->clock)>temp->reidx_time && temp->is_indexing==false)                                                   //checking when index_data is older than the given time and also if there is no indexing performes
        {
            pthread_mutex_unlock(temp->mutex_clock);
            temp->is_indexing=true;                                                                    //changing indexing to true (so that exatcly one pthred will be created)
            if((err=pthread_create(&(temp->pth_id),NULL,thread_work,temp))!=0) ERR("pthread_creat()"); //creating indexing thread (re-indexing) and error handling
            sleep(2);
        }
        else 
        {
...
{
    int fptr;
    if((fptr=TEMP_FAILURE_RETRY(open(path,O_WRONLY|O_CREAT|O_TRUNC,0666)))==-1) ERR("open()");                          //opening the file with O_RDONLY mode (read only)
...
void read_a_file(const char* path,index_structure* struct_idx)
{
    int count=-1;
    int fptr;
    if((fptr=TEMP_FAILURE_RETRY(open(path,O_RDONLY,0666)))==-1) ERR("open()");                          //opening the file with O_RDONLY mode (read only)
    index_data buf;
    while(count!=0)
    {
...
{
    int fptr;                                                                      //variable of file type indicating file
    unsigned char bytes[3]={0};                                                        //variable storing 3 first bytes of file    
    if((fptr=TEMP_FAILURE_RETRY(open(path,O_RDONLY,0666)))==-1) ERR("fopen()");                        //opening file given by path in option "read" and checking for error
    if((reading_buf(fptr,bytes,3))<0) ERR("read()");                                                            //reading 3 first bytes of file
    if((TEMP_FAILURE_RETRY(close(fptr)))==-1) ERR("close()");                                          //closing file
    ...
    return 0;                                                                      //in case of other type
}
...
void count_types(thread_struct* pth,int* _zip,int* _gzip,int* _jpeg,int* _png,int* _dir) //arguments are file type counter pointers (so their value will be changed inside function)
{
    pthread_mutex_lock(pth->mutex_indexing);
    for(int i=0;i<pth->main.indexStruct_size;i++)              //counting each file type in _index by finding which type it is and incrementing appropriate counter
    {
